...
const contentListNoTitle = {
  Comptes_List: <ComptesList refresh={refresh} setRefresh={setRefresh}></ComptesList> ,
  Comptes_To_Approuve: <ComptesToApprouve refresh={refresh} setRefresh={setRefresh}></ComptesToApprouve>,
...
const adminMenu = (
  <Menu>
    ...
    <Menu.Item key="2">
      <a href onClick={()=>{
              axios.delete(`http://localhost:8000/api/admin/logout`,{headers: {"Authorization" : `Bearer ${token}`}}).then(response => {
...
const notificationMenu = (
    <Menu style={{width:'300px'}}>
      ...
      <Notifications  setActiveTabKey2={setActiveTabKey2} refresh={refresh} setRefresh={setRefresh}></Notifications>
    </Menu>
);
  ...
  return (
    <div style={{display:"flex"}}>
      <Card
        ...
        tabBarExtraContent={ <div style={{ display:"flex"}}>
          <div style={{marginRight:"50px"}}>
          <Dropdown overlay={notificationMenu} trigger={['click']}  placement="bottomRight">
          
          <a href onClick={e => e.preventDefault()}>
              <Badge count={nbrCp} size="small" style={{fontSize:"8px"}}>
                <IoMdNotificationsOutline style={{fontSize:"23px",color:"black"}}></IoMdNotificationsOutline>
              </Badge>
          </a>
        ...
        <a href className="ant-dropdown-link" onClick={e => e.preventDefault()} >
           Admin <AiOutlineDown style={{paddingTop:"5px"}}></AiOutlineDown>
        </a>
      </Dropdown>
      </div>}
        onTabChange={key => {
