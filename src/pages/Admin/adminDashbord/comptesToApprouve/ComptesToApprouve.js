...
const ComptesToApprouve = ({setRefresh,refresh}) => {
  ...
  const token=localStorage.getItem('token')
    useEffect(()=>{
    axios.get(`http://localhost:8000/api/admin/comptesToApprouve`,{headers: {"Authorization" : `Bearer ${token}`}}).then(response => {
        let t= []
        response.data.map(x=>{
          let y={
          ...
          />,
            ...
            role:   <> 
                  ...
                  action:<>
                    <Button type="primary"  style={{backgroundColor:"#28B463 ", borderColor:"#28B463 "}} onClick={()=>{
                      let req={
                        role:x.role
                      }
                      axios.put(`http://localhost:8000/api/admin/approuverCompte/${x.id}`,req,{headers: {"Authorization" : `Bearer ${token}`}}).then(response => {
